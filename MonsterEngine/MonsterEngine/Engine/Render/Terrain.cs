...
namespace MonsterEngine.Engine.Render
{
    class Terrain
    {
        ...
        public Terrain(float[,] _HeightMap, float[,] _NormalMap)
        {
        ...
        public void CreateBuffers()
        {
            ...
            for (int x = 0; x < terrainSize; x++)
            {
                ...
                {
                    vertices[x * terrainSize + z] = new Vector3(x / MapGenSize, faHeightMap[x + 1, z + 1], z / MapGenSize);

                    texCoord[x * terrainSize + z] = new Vector2(x / 13f, z / 13f);

                    //texCoord[x * iMapSize + z].Scale(new Vector2(1.6f,1.6f));

                    if (x == 0) vertices[x * terrainSize + z] = new Vector3((x + 1) / MapGenSize, faHeightMap[x + 1, z + 1], z / MapGenSize);
                    if (z == 0) vertices[x * terrainSize + z] = new Vector3((x) / MapGenSize, faHeightMap[x + 1, z + 1], (z + 1) / MapGenSize);
                    if (x == terrainSize - 1) vertices[x * terrainSize + z] = new Vector3((x - 1) / MapGenSize, faHeightMap[x + 1, z + 1], z / MapGenSize);
                    if (z == terrainSize - 1) vertices[x * terrainSize + z] = new Vector3((x) / MapGenSize, faHeightMap[x + 1, z + 1], (z - 1) / MapGenSize);

                    if (x == 0 && z == 0) vertices[x * terrainSize + z] = new Vector3((x + 1) / MapGenSize, faHeightMap[x + 1, z + 1], (z + 1) / 8.0f);
                    if (x == terrainSize - 1 && z == 0) vertices[x * terrainSize + z] = new Vector3((x - 1) / MapGenSize, faHeightMap[x + 1, z + 1], (z + 1) / MapGenSize);
                    if (x == terrainSize - 1 && z == terrainSize - 1) vertices[x * terrainSize + z] = new Vector3((x - 1) / MapGenSize, faHeightMap[x + 1, z + 1], (z - 1) / MapGenSize);
                    if (x == 0 && z == terrainSize - 1) vertices[x * terrainSize + z] = new Vector3((x + 1) / MapGenSize, faHeightMap[x + 1, z + 1], (z - 1) / MapGenSize);
                }
            }
