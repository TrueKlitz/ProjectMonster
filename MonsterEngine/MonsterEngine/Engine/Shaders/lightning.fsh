...
void main(void)
{
	...
	if( parallaxMapping ){texCoords = (texCoord0.xy + (directionToEye * tbnMatrix).xy * texture2D( texHeight , texCoord0.xy ).g * 0.04 );}                
	if( normalMapping ){normal = normalize(tbnMatrix * (2 * texture2D(texNormal, texCoords).xyz - 1));}              
	...
	if(specluar > 0.0){
		...
		spec = specularCoefficient * directionalLightColor;
	}
